...
$_SC['dbpw'] = 'mysql';						//SupeSite数据库密码
$_SC['dbname'] = 'istcms';						//SupeSite数据库名
$_SC['tablepre'] = 'supe_';					//SupeSite表名前缀(不能与论坛的表名前缀相同)
$_SC['pconnect'] = 0;						//SupeSite数据库持久连接 0=关闭, 1=打开
$_SC['dbcharset'] = 'gbk';					//SupeSite数据库字符集

$_SC['siteurl'] = 'http://localhost/ISTCMS';						//SupeSite程序文件所在目录的URL访问地址。可以填写以 http:// 开头的完整URL，也可以填写相对URL。末尾不要加 /。如果程序无法自动获取，请务必手工修改为 http://www.yourwebsite.com/supesite 形式
...
define('UC_DBPW', 'mysql');						// UCenter 数据库密码
define('UC_DBNAME', 'istcms');					// UCenter 数据库名称
define('UC_DBCHARSET', 'gbk');					// UCenter 数据库字符集
define('UC_DBTABLEPRE', '`istcms`.uc_');					// UCenter 数据库表前缀
define('UC_DBCONNECT', '0');					// UCenter 数据库持久连接 0=关闭, 1=打开

// 通信相关
define('UC_KEY', 'Oboey8m7BbGaP5Q7ibF07bI1Y4d9D2ue60v5ubEaEb43A6d4F1UdS7A1r3Y0paB7');						// 与 UCenter 的通信密钥, 要与 UCenter 保持一致
define('UC_API', 'http://localhost/istcms/ucenter');						// UCenter 的 URL 地址, 在调用头像时依赖此常量
