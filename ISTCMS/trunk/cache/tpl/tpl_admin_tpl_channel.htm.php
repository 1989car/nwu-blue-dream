...
<?php if(!$thevalue['type']) { ?>
 checked
<?php } ?>
 onclick="checktype('tr_usesample');readonlyall(this.form, true);" />单个页面&nbsp;&nbsp;
<input name="type" type="radio" value="news"
<?php if($thevalue['type']) { ?>
 checked
<?php } ?>
  onclick="checktype('tr_category');readonlyall(this.form, false);" />文章频道&nbsp;&nbsp;
<?php } else { ?>
<?php $typearr = array('type'=>'内置','system'=>'聚合','model'=>'模型','user'=>'自定义');; ?><?=$typearr[$thevalue['type']]?>
...
<?php empty($class) ? $class=' class="darkrow"': $class='';; ?><?php $typearr = array('type'=>'内置','system'=>'聚合','model'=>'模型','user'=>'自定义');; ?><tr<?=$class?> align="center">
